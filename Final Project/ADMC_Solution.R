...
#*Problem name: Final Assignment_ADMCI
...
train_dataf<-read.csv('E:/Academic/MAIA/Semester_2/Main/Statistical Learning and Data Mining/Final Project/ADMC/ADMCItrain.csv', header = TRUE)
test_dataf<-read.csv('E:/Academic/MAIA/Semester_2/Main/Statistical Learning and Data Mining/Final Project/ADMC/ADMCItest.csv', header = TRUE)
...
#*** Creating Dummy Data for Logistic Regression & Linear Discriminant Analysis
dummy.tdata = dummy.data.frame(tdata)
dummy.tdata = dummy.tdata[,-598]
dummy.vdata = dummy.data.frame(vdata)
dummy.vdata = dummy.vdata[,-598]

#***Applying Logistic Regression
#library(glm)
glm_model = glm(LabelsAD ~ G_SupraMarginal.7.L + G_Angular.1.L + S_Intraparietal.3.R + 
                  S_Sup_Temporal.3.R + G_Temporal_Mid.4.R + G_Precuneus.3.R + 
                  EMILIN1 + HLA.DOA, data = dummy.tdata, family = 'binomial',control = list(maxit = 500))
summary(glm_model)
set.seed(10)
...
#***Applying LDA 
lda_model = lda(LabelsAD ~ G_SupraMarginal.7.L + G_Angular.1.L + S_Intraparietal.3.R + 
                  S_Sup_Temporal.3.R + G_Temporal_Mid.4.R + G_Precuneus.3.R + 
                  EMILIN1 + HLA.DOA, data = dummy.tdata)
set.seed(10)
...
svm_model = svm(Labels ~ G_SupraMarginal.7.L + G_Angular.1.L + S_Intraparietal.3.R + 
                  S_Sup_Temporal.3.R + G_Temporal_Mid.4.R + G_Precuneus.3.R + 
                  EMILIN1 + HLA.DOA, data = tdata, kernel = "linear", cost = 1, scale = TRUE)
summary(svm_model)
set.seed(10)
...
RandomForest.fit = randomForest(Labels ~ G_SupraMarginal.7.L + G_Angular.1.L + S_Intraparietal.3.R + 
                                  S_Sup_Temporal.3.R + G_Temporal_Mid.4.R + G_Precuneus.3.R + 
                                  EMILIN1 + HLA.DOA, data = tdata)
set.seed(10)
...
save(patient_id, pred.logR.predict.test, pred.lda_class.predict.test, pred.svm.predict.test, RF.predict.test_df, file = "E:/Academic/MAIA/Semester_2/Main/Statistical Learning and Data Mining/Final Project/ADMC/file1.RData")
features <- data.frame (train_dataf$G_SupraMarginal.7.L , train_dataf$G_Angular.1.L , train_dataf$S_Intraparietal.3.R , 
                          train_dataf$S_Sup_Temporal.3.R , train_dataf$G_Temporal_Mid.4.R , train_dataf$G_Precuneus.3.R , 
                          train_dataf$EMILIN1 , train_dataf$HLA.DOA)
save(features, file = "E:/Academic/MAIA/Semester_2/Main/Statistical Learning and Data Mining/Final Project/ADMC/file2.RData")
